...
function fixTemplateSEO(){
  ...
  if(!slug) return;
  fetch('https://api.webflow.com/v2/collections/'+match.id+'/items?limit=1',{
    headers:{Authorization:'Bearer '+token}
  }).then(function(r){return r.json();}).then(function(){
    // use slug filter approach - fetch by slug
    return fetch('https://api.webflow.com/v2/collections/'+match.id+'/items?limit=100',{
      headers:{Authorization:'Bearer '+token}
    });
  }).then(function(r){return r.json();}).then(function(d){
    var items=d.items||[];
    ...
    var seoTitle=f['seo-title'];
    var seoDesc=f['seo-description'];
    if(seoTitle){
    ...
    if(seoDesc){
      ...
      else{var m=document.createElement('meta');m.name='description';m.content=seoDesc;document.head.appendChild(m);}
